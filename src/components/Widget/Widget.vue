<template>
  <section :class="{
    ...
    <div v-if="!customControls && mainControls"
      ...
      <a @click="changeState($event, 'fullscreen')" v-if="fullscreen
      && state !== 'fullscreen'" href="#" :id="`fullscreenId-${randomId}`">
      ...
      <a @click="changeState($event, 'default')" v-if="fullscreen
      && state === 'fullscreen'" href="#" :id="`restoreId-${randomId}`">
    ...
    <div v-if="customControls" v-html="customControls"
         ref="customControlsRef" class="widgetControls widget-controls"></div>
...
export default {
  ...
  methods: {
    closeWidget(e) {
      ...
      const { length } = $parentEl.classList;
